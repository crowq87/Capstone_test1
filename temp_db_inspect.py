...
cur = conn.cursor()
cur.execute("SELECT tablename FROM pg_catalog.pg_tables WHERE schemaname='public' AND tablename LIKE 'core_%' ORDER BY tablename;")
print('core_tables:', [row[0] for row in cur.fetchall()])
cur.execute("SELECT app, name FROM django_migrations WHERE app='core' ORDER BY name")
print('core_migrations:', [row for row in cur.fetchall()])
cur.execute("SELECT app, name FROM django_migrations ORDER BY app, name")
print('all_migrations_count:', cur.rowcount)
cur.close()
conn.close()
